  ...
  try {
    ...
    if (!selectedDay || !isStartSelectionAvailable(selectedDay, requestedHours, startHour)) {
      await logBookingRequest({
        ...
        requestId,
        statusCode: 409,
        details: { reason: 'slot_not_available_policy_cache', slug, date, requestedHours, startHour },
      });
    ...
    if (!providerAvailable) {
      await logBookingRequest({
        ...
        requestId,
        statusCode: 409,
        details: { reason: 'slot_not_available_provider', slug, date, requestedHours, startHour },
      });
      return json(req, 409, { error: 'Selected date/time is no longer available', requestId });
    }
    ...
    if (error instanceof CalApiError) {
      ...
      try {
        await logBookingRequest({
          ...
          details: {
            reason: isConflict ? 'cal_conflict' : isCalClientError ? 'cal_client_error' : 'cal_error',
