// Agency OS mock data — port of `Yacht Agency OS - Dani/data.js` to TypeScript.
//
// The prototype anchors "today" to Thu 2026-05-07 so the mock dataset reads
// consistently regardless of when the surface is opened. Keep the anchor — the
// follow-up wire-up doc tracks how each section maps to real Supabase data.
...
export interface Booking {
  ...
  note?: string;
  conflict?: boolean;
  blockKind?: 'mantenimiento' | 'propietario' | 'privado';
  expiresAt?: Date;
}
...
export const TZ_LABEL = 'GMT-7 · Mazatlán';
...
export const yById: Record<string, Yacht> = Object.fromEntries(
  yachts.map((y) => [y.id, y])
);
...
export function day(offset: number, h = 0, m = 0): Date {
...
export function bookingsFor(scenario: Scenario): Booking[] {
  ...
  if (scenario === 'quiet') {
    return B.filter((b) => b.type === 'block');
  }

  return B;
}

export function alertsFor(scenario: Scenario): Alert[] {
  if (scenario === 'quiet') {
    return [
...
export function todayBookings(scenario: Scenario): Booking[] {
  return bookingsFor(scenario)
    .filter((b) => b.start.getDate() === TODAY.getDate() && b.type === 'reservation')
    .sort((a, b) => a.start.getTime() - b.start.getTime());
}

export function revenue(scenario: Scenario): Revenue {
...
export function utilization(yachtId: string, scenario: Scenario): number[] {
  let seed = (yachtId + scenario)
    .split('')
...
const usdK = (n: number) => '$' + (n / 1000).toFixed(n >= 10000 ? 0 : 1) + 'k';
...
export const fmt = {
  usd, usdK, pct, pad, hhmm, ddmm, longDate, ago, dow, dowFull, monthAbbr,
};
