// Agency OS theme — port of `Yacht Agency OS - Dani/theme.jsx`. Marina-only
// (UniqueOS warm-cream variant); Bridge/Concierge are skipped per the handoff.
//
// Inline styles are preserved verbatim from the prototype to keep the surfaces
// pixel-perfect with the source HTML. `theme` is passed explicitly into each
// primitive — they're called from one component depth, so no context needed.

import { CSSProperties, MouseEventHandler, ReactNode } from 'react';
import type { Yacht } from '@/lib/agency-os/data';
...
export const marinaTheme: AgencyTheme = {
  id: 'marina',
  label: 'UniqueOS',
  sub: 'Cabo · Operaciones',
  isDark: false,
...
type PillTone = 'muted' | 'accent' | 'success' | 'warn' | 'danger' | 'info' | 'solid';
...
export function Pill({ theme: t, tone = 'muted', children, mono, sm, style }: PillProps) {
  const map: Record<PillTone, { color: string; bg: string; border: string }> = {
...
export function Stat({ theme: t, label, value, sub, mono, big, accentValue }: StatProps) {
  return (
...
export function Spark({ theme: t, values, w = 80, h = 18, fill }: SparkProps) {
...
export function BarSeries({ theme: t, values, w = 120, h = 28, color }: BarSeriesProps) {
...
export function SectionHd({ theme: t, title, count, right, level = 1 }: SectionHdProps) {
  return (
...
export function Kbd({ theme: t, children, dim }: KbdProps) {
  return (
    <span style={{
...
export function Avatar({ theme: t, name, size = 24 }: AvatarProps) {
  const initials = name
    .replace(/[^\p{L}\s]/gu, '')
    .split(/\s+/)
    .filter(Boolean)
...
export function YachtMark({ yacht, w = 4, h = 16 }: YachtMarkProps) {
  return (
    <span style={{
...
export function SourceDot({ theme: t, source }: SourceDotProps) {
  const map: Record<string, string> = {
    experiencia: t.accent,
    reserva: t.success,
    abierta: t.info,
  };
...
export function Priority({ theme: t, priority }: PriorityProps) {
...
export function Btn({ theme: t, children, primary, sm, style, onClick, kbd, mono }: BtnProps) {
  return (
...
export function Card({ theme: t, children, style, padded = true, glow }: CardProps) {
  return (
    <div style={{
...
export function YachtPhoto({ theme: t, yacht, w, h, label }: YachtPhotoProps) {
