  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card">
      <div className="overflow-x-auto">
        <div className="min-w-[980px]">
          <div
            ...
            style={{ gridTemplateColumns: `72px repeat(${dates.length}, minmax(130px, 1fr))` }}
          >
            ...
            {dates.map((date, index) => {
              ...
              return (
                <div key={dateKeys[index]} className="border-r border-border/70 px-2 py-2 text-xs font-semibold text-muted-foreground last:border-r-0">
                  {dayLabel}
                </div>
              );
            })}
          ...
          >
            ...
            {dateKeys.map((dateKey) => {
              ...
              return (
                <div key={dateKey} className="relative border-r border-border/70 last:border-r-0">
                  {Array.from({ length: totalHours + 1 }).map((_, index) => (
                    <div
                      key={index}
                  ...
                  {dayEvents.map((event) => {
                    ...
                    const rawEndMinutes =
                      end.dateKey === start.dateKey ? end.hour * 60 + end.minute : hourEnd * 60;

                    const clampedStart = Math.max(startMinutes, hourStart * 60);
                  ...
                  {nowMarker && nowMarker.dateKey === dateKey && nowMarker.minutes >= hourStart * 60 && nowMarker.minutes <= hourEnd * 60 ? (
                    <div
                      className="absolute left-0 right-0 z-30 border-t border-rose-500"
                      style={{ top: (nowMarker.minutes - hourStart * 60) * pxPerMinute }}
                    />
                  ) : null}
                </div>
              );
      ...
      {!loading && events.length === 0 ? (
        <div className="border-t border-border/70 p-4 text-sm text-muted-foreground">{copy.empty}</div>
      ) : null}
    </div>
  );
}
