...
const COPY = {
  en: {
    ...
    endpointNotReady:
      "Internal calendar endpoint is not deployed yet. Ask backend to deploy internal-calendar-bookings.",
    ...
    staleSlotError:
      "That slot is no longer available. Choose another date or start time.",
    actionSuccessRescheduledTitle: "Reservation updated",
    actionSuccessRescheduledDescription:
      "The reservation was updated successfully.",
    actionSuccessRemovedTitle: "Reservation removed",
    actionSuccessRemovedDescription:
      "The reservation was removed successfully.",
    gmtLabel: "GMT",
    horizontalScrollHint:
      "Scroll horizontally to see each day in timeline view.",
    agendaLabel: "Agenda",
    ...
    drawer: {
      ...
      actionDeckDescription:
        "Manage this yacht reservation proactively from here.",
      ...
      removeDescription:
        "This will cancel the reservation for the selected trip.",
      ...
      actionUnavailable:
        "This reservation does not include a booking UID, so actions are unavailable.",
      ...
      removeReasonRequired:
        "Please provide a reason before removing this reservation.",
  ...
  es: {
    ...
    day: "Día",
    emptyYachts:
      "No hay yates elegibles configurados para la vista de calendario.",
    ...
    endpointNotReady:
      "El endpoint interno de calendario no está desplegado. Pide al backend desplegar internal-calendar-bookings.",
    actionsEndpointNotReady:
    ...
    staleSlotError:
      "Ese horario ya no está disponible. Elige otra fecha u hora de inicio.",
    ...
    gmtLabel: "GMT",
    horizontalScrollHint:
      "Desliza horizontalmente para ver cada día en la línea de tiempo.",
    agendaLabel: "Agenda",
    reservationEndpointNotReady:
    ...
    drawer: {
      ...
      lockedReadOnly:
        "Modo solo lectura: solo admins pueden editar los detalles.",
      attendee: "Asistente",
      ...
      actionDeckDescription:
        "Gestiona esta reserva del yate de forma proactiva desde aquí.",
      ...
      removeReasonPlaceholder:
        "Escribe un motivo claro para el registro del equipo...",
      ...
      actionUnavailable:
        "Esta reserva no incluye booking UID, por lo que las acciones no están disponibles.",
      actionErrorFallback:
        "No se pudo completar esta acción. Intenta de nuevo.",
      ...
      duration5:
        "Los viajes de 5+ horas pueden iniciar en cualquier hora que termine antes de las 6:00 PM.",
...
const buildFallbackReservationRecord = (
...
const normalizeReservationRecord = (
  payload: ReservationDetailsApiResponse,
  event: AdminCalendarEvent,
): ReservationRecord => {
  ...
  const auditRows = Array.isArray(auditRaw)
    ? auditRaw.filter(
  ...
  const reservation: ReservationDetails = {
    ...
    bookingUidHistory: asStringArray(
      reservationRow.bookingUidHistory ?? reservationRow.booking_uid_history,
    ),
    yachtSlug:
      asString(reservationRow.yachtSlug ?? reservationRow.yacht_slug) ||
      fallback.reservation.yachtSlug,
    yachtName:
      asString(reservationRow.yachtName ?? reservationRow.yacht_name) ||
      fallback.reservation.yachtName,
    startAt:
      asString(reservationRow.startAt ?? reservationRow.start_at) ||
      fallback.reservation.startAt,
    endAt:
      asString(reservationRow.endAt ?? reservationRow.end_at) ||
      fallback.reservation.endAt,
    status: asString(reservationRow.status) || fallback.reservation.status,
    guestProfileId: asString(
      reservationRow.guestProfileId ?? reservationRow.guest_profile_id,
    ),
    guestCount: asNumber(
      reservationRow.guestCount ?? reservationRow.guest_count,
    ),
    adultCount: asNumber(
      reservationRow.adultCount ?? reservationRow.adult_count,
    ),
    kidsCount: asNumber(reservationRow.kidsCount ?? reservationRow.kids_count),
    kidsNotes:
      asString(reservationRow.kidsNotes ?? reservationRow.kids_notes) || "",
    stayingMultiplePlaces: asBoolean(
      reservationRow.stayingMultiplePlaces ??
        reservationRow.staying_multiple_places,
    ),
    allergies: asStringArray(reservationRow.allergies),
    preferences: asStringArray(reservationRow.preferences),
    dietaryNotes:
      asString(reservationRow.dietaryNotes ?? reservationRow.dietary_notes) ||
      "",
    mobilityNotes:
      asString(reservationRow.mobilityNotes ?? reservationRow.mobility_notes) ||
      "",
    occasionNotes:
      asString(reservationRow.occasionNotes ?? reservationRow.occasion_notes) ||
      "",
    conciergeNotes:
      asString(
        reservationRow.conciergeNotes ?? reservationRow.concierge_notes,
      ) || "",
    internalNotes:
      asString(reservationRow.internalNotes ?? reservationRow.internal_notes) ||
      fallback.reservation.internalNotes,
  ...
  const guest: GuestProfile = {
    ...
    fullName:
      asString(guestRow.fullName ?? guestRow.full_name) ||
      fallback.guest.fullName,
    preferredName:
      asString(guestRow.preferredName ?? guestRow.preferred_name) || "",
    ...
    preferredLanguage:
      asString(guestRow.preferredLanguage ?? guestRow.preferred_language) || "",
  ...
  const stays: ReservationStay[] = Array.isArray(payload.stays)
    ? payload.stays.map((item, index) => {
        ...
        return {
          ...
          locationLabel:
            asString(stay.locationLabel ?? stay.location_label) || "",
          checkInDate: asString(stay.checkInDate ?? stay.check_in_date) || "",
          checkOutDate:
            asString(stay.checkOutDate ?? stay.check_out_date) || "",
  ...
  const auditSummary: ReservationAuditSummary | null = auditRow
    ? {
        ...
        lastAction: asString(
          auditRow.lastAction ?? auditRow.last_action ?? auditRow.action,
        ),
        lastActionAt: asString(
          auditRow.lastActionAt ??
            auditRow.last_action_at ??
            auditRow.created_at,
        ),
        lastActorUserId: asString(
          auditRow.lastActorUserId ??
            auditRow.last_actor_user_id ??
...
const normalizeEvents = (value: unknown): AdminCalendarEvent[] => {
  ...
  return value
    .map((item): AdminCalendarEvent | null => {
      ...
      const startIso =
        asString(row.startIso) ||
        asString(row.start) ||
        asString(row.startTime);
      const endIso =
        asString(row.endIso) || asString(row.end) || asString(row.endTime);
      ...
      return {
        ...
        attendeeEmail:
          asString(row.attendeeEmail) || asString(row.attendee_email),
        attendeePhone:
...
export default function Calendar() {
  ...
  const [drawerAvailabilityDays, setDrawerAvailabilityDays] = useState<
    Record<string, DayAvailability>
  >({});
  const [drawerAvailabilityLoading, setDrawerAvailabilityLoading] =
    useState(false);
  ...
  const [nowMarker, setNowMarker] = useState<TimeMarker>(() =>
    getNowMarker(BOOKING_TIMEZONE),
  );
  ...
  const todayDateKey = useMemo(
    () => toTimeZoneParts(new Date().toISOString(), BOOKING_TIMEZONE).dateKey,
    [],
  );
  ...
  const rangeEnd = useMemo(
    () => format(visibleDates[visibleDates.length - 1], "yyyy-MM-dd"),
    [visibleDates],
  );
  ...
  const loadYachts = useCallback(async () => {
    ...
    try {
      ...
      setSelectedYachtSlug(
        (current) => current || eligibleYachts[0]?.slug || null,
      );
    } catch (loadError) {
  ...
  const loadEvents = useCallback(
    async (silent = false): Promise<AdminCalendarEvent[]> => {
      ...
      try {
        ...
        setError(
          loadError instanceof Error ? loadError.message : copy.defaultError,
        );
        setEvents([]);
        return [];
      } finally {
  ...
  const loadDrawerAvailability = useCallback(
    async ({
      ...
      try {
        ...
        const payload = (await response
          .json()
          .catch(() => ({}))) as AvailabilityApiResponse;
        ...
        console.error(
          "Failed to load reschedule availability:",
  ...
  const loadReservationDetails = useCallback(
    async ({
      ...
      try {
        ...
        const payload = (await response
          .json()
          .catch(() => ({}))) as ReservationDetailsApiResponse;
    ...
    [
      ...
      copy.reservationEndpointNotReady,
      copy.sessionExpired,
    ],
  );
  ...
  useEffect(() => {
    ...
    setSelectedYachtSlug((current) =>
      current === deepLinkSlug ? current : deepLinkSlug,
    );
  }, [searchParams, yachts]);
  ...
  useEffect(() => {
    ...
    setViewMode((current) => (current === "week" ? "day" : current));
  }, [isMobile]);
  ...
  const handleEventClick = useCallback(
    (event: AdminCalendarEvent) => {
      ...
      const eventDate = parseDateKeyToDate(
        toTimeZoneParts(event.startIso, BOOKING_TIMEZONE).dateKey,
      );
      setDrawerMonthDate(eventDate);
      void loadDrawerAvailability({
        monthDate: eventDate,
        yachtSlug: event.yachtSlug,
      });
  ...
  const handleDrawerPreviousMonth = () => {
    setDrawerMonthDate((current) => {
      ...
      if (selectedEvent?.yachtSlug) {
        void loadDrawerAvailability({
          monthDate: next,
          yachtSlug: selectedEvent.yachtSlug,
        });
      }
      return next;
    });
  ...
  const handleSubmitReschedule = useCallback(
    async ({
      ...
      try {
        ...
        const response = await fetch(
          `${apiBase}/internal-calendar-booking-reschedule`,
          {
            method: "POST",
            headers: {
              apikey: anonKey,
        ...
        const payload = (await response
          .json()
          .catch(() => ({}))) as CalendarActionApiResponse;
        ...
        if (!response.ok) {
          if (isEndpointUnavailable(response.status, payload.error)) {
            return {
              ...
              error: copy.actionsEndpointNotReady,
            };
          }
          if (response.status === 401) {
            return { ok: false, status: 401, error: copy.sessionExpired };
          ...
          if (response.status === 409) {
            ...
            return {
              ok: false,
              status: 409,
              error: payload.error || copy.staleSlotError,
            };
          }
          return {
            ok: false,
            status: response.status,
            error: payload.error || copy.defaultError,
          };
        }
        ...
        const nextEvent =
          refreshedEvents.find(
            (item) =>
              item.bookingUid === nextBookingUid || item.id === nextBookingUid,
          ) ||
          refreshedEvents.find(
            (item) => item.bookingUid === bookingUid || item.id === bookingUid,
          ) ||
          null;
  ...
  const handleSubmitCancel = useCallback(
    async ({
      ...
      try {
        ...
        const response = await fetch(
          `${apiBase}/internal-calendar-booking-cancel`,
          {
            method: "POST",
            headers: {
              apikey: anonKey,
    ...
    async ({ bookingUid, slug }: { bookingUid: string; slug: string }) => {
      const activeEvent =
        selectedEvent && selectedEvent.yachtSlug === slug
          ...
          : events.find(
              (event) =>
                event.bookingUid === bookingUid || event.id === bookingUid,
            ) || null;
  ...
  const handleSaveReservationDetails = useCallback(
    async ({
      ...
      try {
        ...
        const response = await fetch(
          `${apiBase}/internal-reservation-details`,
          {
            method: "PUT",
            headers: {
              apikey: anonKey,
            ...
            body: JSON.stringify({
              slug: event.yachtSlug,
              bookingUid:
                record.reservation.bookingUidCurrent ||
                event.bookingUid ||
                event.id,
              reservation: {
        ...
        if (!response.ok) {
          if (isEndpointUnavailable(response.status, payload.error)) {
            return {
              ok: false,
              status: response.status,
              error: copy.reservationEndpointNotReady,
            };
          }
          if (response.status === 401) {
            return { ok: false, status: 401, error: copy.sessionExpired };
        ...
        return {
          ok: true,
          status: 200,
          bookingUid: record.reservation.bookingUidCurrent || event.bookingUid,
        };
      } catch (saveError) {
        return {
          ok: false,
          status: 500,
          error:
            saveError instanceof Error ? saveError.message : copy.defaultError,
        };
      }
    },
    [
  ...
  const handleExportReservationDetails = useCallback(
    async ({
      ...
      try {
        ...
        const response = await fetch(
          `${apiBase}/internal-reservation-details/export`,
          {
            method: "POST",
            headers: {
              apikey: anonKey,
        ...
        const payload = (await response
          .json()
          .catch(() => ({}))) as ReservationExportApiResponse;
        ...
        return {
          ...
          fileName:
            payload.fileName ||
            `reservation-${record.reservation.bookingUidCurrent || event.id}.csv`,
        };
      } catch (exportError) {
        return {
          ok: false,
            ...
            {viewMode === "month" ? (
              ...
              <TimeGrid
                ...
                copy={{
                  ...
                  empty: copy.emptyEvents,
                  gmtLabel,
                  horizontalScrollHint: copy.horizontalScrollHint,
